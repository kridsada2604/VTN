...
export default async function Page({ searchParams }: { searchParams: Promise<{ edit?: string }> }) {
  ...
  const rows = await getInventoryMasterRows("category");
    ...
    <div>
      ...
      <div className="two-column-page mt-6">
                ...
                <tr key={row.id}>
                  ...
                  <td>
                    <div className="action-row">
                      <Link className="btn-secondary btn-small" href={"/inventory/categories?edit=" + row.id}>?????</Link>
        ...
        <FormCard title={editing ? "???????????????????" : "???????????????????"}>
          <form action={saveCategory} className="form-grid">
            ...
            <div className="full action-row">
              ...
              {editing && <Link className="btn-secondary" href="/inventory/categories">??????</Link>}
            </div>
          </form>
        </FormCard>
      </div>
