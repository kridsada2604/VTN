...
export class CustomerRepository {
  ...
  async list(companyId: string, search: string) {
    let query = this.supabase.from("customers").select("id,code,name,tax_id,phone,email,address,is_active").eq("company_id", companyId).order("code");
  ...
  async save(companyId: string, input: MasterPartyInput) {
    ...
    const result = input.id
      ? await this.supabase.from("customers").update(payload).eq("id", input.id).eq("company_id", companyId)
      : await this.supabase.from("customers").insert(payload);
    if (result.error) throw result.error;
  }

  async setActive(companyId: string, input: ToggleMasterPartyInput) {
    const { error } = await this.supabase.from("customers").update({ is_active: input.is_active }).eq("id", input.id).eq("company_id", companyId);
    if (error) throw error;
  }
}
