  ...
  return (
    <form action={action} className="card p-5 space-y-5">
      ...
      <div className="form-grid">
        ...
        <label>
          <span className="label">วันที่ใบแจ้งหนี้ *</span>
          <input className="input" type="date" name="invoice_date" required defaultValue={today} />
        </label>
        <label>
        ...
        <label className="full">
          <span className="label">เงื่อนไขการชำระ</span>
                ...
                <tr key={index}>
                  ...
                  <td>
                    <input
                      ...
                      onChange={(event) => patchItem(index, { unit_price: Number(event.target.value) })}
                    />
                  </td>
                  <td>
                    <input
                      ...
                      onChange={(event) => patchItem(index, { discount_percent: Number(event.target.value) })}
                    />
                  </td>
                  <td>
                    <input
                      ...
                      step="0.01"
                      value={item.tax_rate}
                      onChange={(event) => patchItem(index, { tax_rate: Number(event.target.value) })}
                    />
                  </td>
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_360px]">
        ...
        <div className="rounded-2xl bg-slate-50 p-5 space-y-2">
          <div className="flex justify-between">
            ...
            <b>฿{totals.subtotal.toLocaleString("th-TH", { minimumFractionDigits: 2 })}</b>
          </div>
          <div className="flex justify-between">
            <span>ส่วนลด</span>
            <b>฿{totals.discount.toLocaleString("th-TH", { minimumFractionDigits: 2 })}</b>
          </div>
          <div className="flex justify-between">
            <span>ภาษี</span>
            <b>฿{totals.tax.toLocaleString("th-TH", { minimumFractionDigits: 2 })}</b>
          </div>
          <div className="mt-3 flex justify-between border-t pt-3 text-xl">
            <span className="font-black">ยอดสุทธิ</span>
            <b>฿{totals.total.toLocaleString("th-TH", { minimumFractionDigits: 2 })}</b>
          </div>
        </div>
      </div>

      <div className="action-row">
        <button className="btn-primary">บันทึกใบแจ้งหนี้</button>
      </div>
    </form>
  );
}
