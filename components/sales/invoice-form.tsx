  ...
  return (
    <form action={action} className="card p-5 space-y-5">
      ...
      <div className="form-grid">
        ...
        <label><span className="label">VAT Status</span><select className="input" value={String(isVatRegistered)} onChange={(event) => { const registered = event.target.value === "true"; setIsVatRegistered(registered); setItems((current) => current.map((item) => ({ ...item, tax_rate: registered ? taxDefaults.default_vat_rate : 0 }))); }}><option value="true">VAT registered</option><option value="false">Not VAT registered</option></select></label>
      ...
      <div className="table-wrap"><table className="data-table"><thead><tr><th>Product</th><th>Description</th><th>Qty</th><th>Price</th><th>Discount %</th><th>VAT %</th><th>Total</th><th /></tr></thead><tbody>{items.map((item, index) => { const subtotal = item.quantity * item.unit_price; const discount = subtotal * item.discount_percent / 100; const tax = isVatRegistered ? (subtotal - discount) * item.tax_rate / 100 : 0; const total = subtotal - discount + tax; return <tr key={index}><td><select className="input" value={item.product_id} onChange={(event) => { const product = products.find((entry) => entry.id === event.target.value); patchItem(index, { product_id: event.target.value, description: product?.name ?? "", unit_price: Number(product?.selling_price ?? 0) }); }}><option value="">No product link</option>{products.map((product) => <option key={product.id} value={product.id}>{product.sku} - {product.name}</option>)}</select></td><td><input className="input" value={item.description} onChange={(event) => patchItem(index, { description: event.target.value })} /></td><td><input className="input" type="number" min="0.0001" step="0.0001" value={item.quantity} onChange={(event) => patchItem(index, { quantity: Number(event.target.value) })} /></td><td><input className="input" type="number" min="0" step="0.01" value={item.unit_price} onChange={(event) => patchItem(index, { unit_price: Number(event.target.value) })} /></td><td><input className="input" type="number" min="0" max="100" step="0.01" value={item.discount_percent} onChange={(event) => patchItem(index, { discount_percent: Number(event.target.value) })} /></td><td><input className="input" type="number" min="0" step="0.01" disabled={!isVatRegistered} value={item.tax_rate} onChange={(event) => patchItem(index, { tax_rate: Number(event.target.value) })} /></td><td className="font-bold">THB {total.toLocaleString("th-TH", { minimumFractionDigits: 2 })}</td><td><button type="button" className="btn-secondary btn-small" disabled={items.length === 1} onClick={() => setItems((current) => current.filter((_, itemIndex) => itemIndex !== index))}>Remove</button></td></tr>; })}</tbody></table></div>
