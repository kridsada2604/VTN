  ...
  return (
    <form action={async (fd) => { localStorage.removeItem(draftKey); await action(fd); }} className="card p-5 space-y-5">
      ...
      <div className="form-grid">
        ...
        {draft.is_vat_registered && <label><span className="label">Sales VAT</span><input className="input" readOnly value={`${taxDefaults.default_vat_rate}%`} /></label>}
      ...
      <div className="table-wrap"><table className="data-table"><thead><tr><th>Product</th><th>Description</th><th>Qty</th><th>Price</th><th>Discount %</th>{draft.is_vat_registered && <th>VAT %</th>}<th>Total</th><th /></tr></thead><tbody>{draft.items.map((item, index) => { const subtotal = item.quantity * item.unit_price; const discount = subtotal * item.discount_percent / 100; const tax = draft.is_vat_registered ? (subtotal - discount) * item.tax_rate / 100 : 0; const total = subtotal - discount + tax; return <tr key={index}><td><select className="input" value={item.product_id} onChange={(event) => { const product = products.find((entry) => entry.id === event.target.value); patchItem(index, { product_id: event.target.value, description: product?.name || "", unit_price: Number(product?.selling_price || 0) }); }}><option value="">No product link</option>{products.map((product) => <option key={product.id} value={product.id}>{product.sku} - {product.name}</option>)}</select></td><td><input className="input" value={item.description} onChange={(event) => patchItem(index, { description: event.target.value })} /></td><td><input className="input" type="number" min="0.0001" step="0.0001" value={item.quantity} onChange={(event) => patchItem(index, { quantity: Number(event.target.value) })} /></td><td><input className="input" type="number" min="0" step="0.01" value={item.unit_price} onChange={(event) => patchItem(index, { unit_price: Number(event.target.value) })} /></td><td><input className="input" type="number" min="0" max="100" step="0.01" value={item.discount_percent} onChange={(event) => patchItem(index, { discount_percent: Number(event.target.value) })} /></td>{draft.is_vat_registered && <td><input className="input" type="number" min="0" step="0.01" value={item.tax_rate} onChange={(event) => patchItem(index, { tax_rate: Number(event.target.value) })} /></td>}<td className="font-bold">THB {total.toLocaleString("th-TH", { minimumFractionDigits: 2 })}</td><td><button type="button" className="btn-secondary btn-small" onClick={() => setDraft((current) => ({ ...current, items: current.items.filter((_, itemIndex) => itemIndex !== index) }))} disabled={draft.items.length === 1}>Remove</button></td></tr>; })}</tbody></table></div>
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_360px]"><label><span className="label">Notes</span><textarea className="input textarea min-h-36" name="notes" value={draft.notes} onChange={(event) => field("notes", event.target.value)} /></label><div className="rounded-2xl bg-slate-50 p-5 space-y-2"><div className="flex justify-between"><span>Subtotal</span><b>THB {totals.subtotal.toLocaleString("th-TH", { minimumFractionDigits: 2 })}</b></div><div className="flex justify-between"><span>Discount</span><b>THB {totals.discount.toLocaleString("th-TH", { minimumFractionDigits: 2 })}</b></div>{draft.is_vat_registered && <div className="flex justify-between"><span>VAT</span><b>THB {totals.tax.toLocaleString("th-TH", { minimumFractionDigits: 2 })}</b></div>}<div className="flex justify-between"><span>Grand Total</span><b>THB {totals.grandTotal.toLocaleString("th-TH", { minimumFractionDigits: 2 })}</b></div><div className="flex justify-between"><span>WHT</span><b>THB {totals.withholding.toLocaleString("th-TH", { minimumFractionDigits: 2 })}</b></div><div className="mt-3 flex justify-between border-t pt-3 text-xl"><span className="font-black">Net Payable</span><b>THB {totals.netPayable.toLocaleString("th-TH", { minimumFractionDigits: 2 })}</b></div></div></div>
